...
$$\mathcal{L}_{\text{distill}} = -\sum_{i=1}^{256} P_T(x_i\left\vert x_c\right.) \log P_S(x_i\left\vert x_c\right.)$$

여기서 $P_T(x_i\left\vert x_c\right.)$는 교사 모델의 확률 분포이고, $P_S(x_i\left\vert x_c\right.)$는 학생 모델의 확률 분포입니다. 교사의 목표 분포는 샘플링되지 않은 로짓에 대해 0 확률로 설정되고 재정규화됩니다.
