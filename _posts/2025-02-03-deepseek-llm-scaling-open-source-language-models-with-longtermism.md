...
모델 평가 단계에서는 생성 기반 태스크와 비생성 태스크에 대해 서로 다른 최적화 전략을 적용했습니다. 텍스트 생성이 필요한 태스크에는 vLLM(Efficient Memory Management for Large Language Model Serving)을 사용하여 KV 캐시를 효율적으로 관리하고 생성 속도를 향상시켰습니다. 반면 분류나, 회귀와 같은 비생성 태스크에는 연속 배칭(continuous batching) 기법을 도입하여 다양한 길이의 입력을 효율적으로 처리했습니다. 이 접근법은 수동으로 배치 크기를 조정할 필요가 없고 불필요한 패딩 토큰을 줄여 평가 과정의 처리량을 높이고 메모리 효율성을 개선했습니다.
...
![배치 크기와 학습률 관계](https://ar5iv.labs.arxiv.org//html/2401.02954/assets/figures/loss_bs_lr_1e17.png)

실험 결과는 일반화 오차가 배치 크기와 학습률의 선택에 있어 넓은 범위에서 안정적으로 유지된다는 것을 보여줍니다. 이는 상대적으로 넓은 파라미터 공간 내에서 최적에 가까운 성능을 달성할 수 있다는 것을 의미합니다.

![1e20 FLOPs에서의 최적 하이퍼파라미터](https://ar5iv.labs.arxiv.org//html/2401.02954/assets/figures/loss_bs_lr_1e20.png)
...
![배치 크기 스케일링 곡선](https://ar5iv.labs.arxiv.org//html/2401.02954/assets/figures/flops_bsz_fitting.png)

![학습률 스케일링 곡선](https://ar5iv.labs.arxiv.org//html/2401.02954/assets/figures/flops_lr_fitting.png)
