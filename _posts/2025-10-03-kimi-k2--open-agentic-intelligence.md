...
SWE-bench Verified 및 Multi-SWE-bench 실험 결과 평가에서 귀중한 지원을 제공한 OpenHands 및 Multi-SWE-bench 팀에 감사를 표합니다.
## 부록
...
로짓 폭발은 최대 사전 소프트맥스 어텐션 점수 $S_{\max} = \max_{i,j}(q_i \cdot k_j)$가 훈련 중 무한정 증가할 때 발생합니다. $|q_i \cdot k_j| \leq \|q_i\|\|k_j\| \leq \|x_i\|\|x_j\|\|\mathbf{W}_q\|\|\mathbf{W}_k\|$이고 RMS-Norm이 $\|x_i\|\|x_j\|$를 제한된 상태로 유지하므로, 이 현상은 주로 $\mathbf{W}_q$ 또는 $\mathbf{W}_k$의 증가하는 스펙트럴 노름에 의해 주도됩니다.
